...
xresize(int col, int row) {
	xw.tw = MAX(1, 2*X_MARGIN + col * xw.cw);
	xw.th = MAX(1, 2*Y_MARGIN + row * xw.ch);
}
...
#ifdef MIYOOMINI
		if(xw.win) GFX_FreeSurface(xw.win);
		GFX_Quit();
#else
...
sdlinit(void) {
	...
	if(xw.isfixed) {
		...
		/* window - default size */
		xw.h = 2*Y_MARGIN + term.row * xw.ch;
		xw.w = 2*X_MARGIN + term.col * xw.cw;
		xw.fx = 0;
		xw.fy = 0;
	}
	...
	xw.h = initial_height;
	GFX_Init();
	if(!(screen = GFX_SetVideoMode(xw.w, xw.h, 16, SDL_SWSURFACE))) {
...
#ifdef MIYOOMINI
    xw.win = GFX_CreateRGBSurface(SDL_SWSURFACE, xw.w, xw.h, 16, 0xF800, 0x7E0, 0x1F, 0);	// console screen
#else
...
xdraws(char *s, Glyph base, int x, int y, int charlen, int bytelen) {
	int winx = X_MARGIN + x * xw.cw, winy = Y_MARGIN + y * xw.ch,
	    width = charlen * xw.cw;
	...
	if(y == 0)
		xclear(winx, 0, winx + width, Y_MARGIN);
	if(y == term.row-1)
...
cresize(int width, int height)
{
	...
	col = (xw.w - 2*X_MARGIN) / xw.cw;
	row = (xw.h - 2*Y_MARGIN) / xw.ch;
...
run:
    setlocale(LC_CTYPE, "");
    tnew(initial_width / X_STEP, initial_height / Y_STEP);
    ttynew();
