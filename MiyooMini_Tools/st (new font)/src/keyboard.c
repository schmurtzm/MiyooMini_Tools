...
void draw_keyboard(SDL_Surface* surface) {
	...
	for(int i = 0; i < NUM_KEYS && syms[0][0][i]; i++) {
		total_length += (1 + strlen(syms[0][0][i])) * X_STEP;
	}
	int center_x = (surface->w - total_length) / 2;
	int x = center_x, y = surface->h - Y_STEP * (NUM_ROWS) - (Y_STEP * 2) - 2;
	if (location == 1) y = (Y_STEP * 2);

	SDL_Rect rect = {x - 1, y - 1, total_length + 4 - (X_STEP-X_SIZE), NUM_ROWS * Y_STEP + 3};
	...
	for(int j = 0; j < NUM_ROWS; j++) {
		...
		for(int i = 0; i < row_length[j]; i++) {
			...
			SDL_Rect r2 = {x + 1, y + 1, length * X_STEP + X_SIZE - 1, Y_SIZE};
			if(toggled[j][i]) {
			...
			draw_string(surface, syms[shifted][j][i], x + (X_STEP/2) + 1, y + 1, text_color);
			x += X_STEP * (length + 1);
		}
		y += Y_STEP;
	}
...
//			simulate_key(SDLK_DOWN, STATE_TYPED);
			...
			if(selected_j == 3 && selected_i == 0) selected_j++; // HACK: caps -> shift
